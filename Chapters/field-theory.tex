...
partition function. We will also describe how these variables can be used to transform the
...
\end{equation}
provided that the generator $\xi_{\alpha_m}$ is in the number, and $0$ otherwise. The factors of $(-1)$ comes from anti-commuting
...
\begin{equation}
    ...
    \int\!\mathrm{d}\xi\;1 = 0.
\end{equation}
If the integral consists of several differentials of generators, then these differentials also have to be anti-commuted such that $\mathrm{d}\xi_1\mathrm{d}\xi_2 = -\mathrm{d}\xi_2\mathrm{d}\xi_1$.
...
\end{equation}
In this equation, the indices $\gamma$ and $\delta$ are arbitrary collections of quantum numbers needed to specify a state other than spin,
...
what to do when a spin-dependent action cannot be written on the form in Eq.~\eqref{eq:Field:Ferm:Nambu:terms}.
...
form, and $S$ is now a $2n\times2n$ matrix and not an $n\times n$ matrix. This means that we cannot use the integral in Eq.~\eqref{eq:Field:Ferm:Nambu:quadraticGrassmannInt}
...
discretizing the imaginary time into slices which is usually done through the Matsubara-frequency formalism.
...
\end{equation}
In this equation, it is important to remember that $S'$ is the row-switched matrix of an antisymmetric matrix $S$.
...
integration. We consider the complex contour integral along a path $\mathcal{C}$ running counterclockwise around the complex plane infinitesimally close to
...
\end{tikzpicture}
\caption{Integration contour for the Matsubara sum $\sum_nh(if_n)$. The contour is marked by a solid line and is imagined continuing
to $\pm i\infty$.
The crosses along the imaginary axis symbolize the simple poles of the Fermi-Dirac distribution-function.
...
a Gra\ss mann variable, since it makes it possible to consider low energy excitations of the theory using \eg\ a saddle-point
...
coefficients that could contain Gra\ss mann- or complex variables. Then
\begin{equation}
...
Since $\v{J}$ usually represents some field in a field theory, the new $\v{z}$ is called the auxiliary- or conjugate
field because of its linear coupling to $\v{J}$.
In the less general case that $A$ is a Hermitian matrix, this formula is proved simply by completing the square, then
...
\end{equation}
where $\xi_\alpha$ are Gra\ss mann variables, which can be sketched in the way of the single-vertex diagram in Figure~\ref{fig:Field:HS:twoBodyInteraction}.
\begin{figure}[h]
    \begin{center}
...
\end{equation}
where the auxiliary field $\v{x}$ now is a \emph{real} conjugate field.
...
through a subsequent stationary phase approximation. By diagonalizing the interaction potential $\hat{V}$ into its different
...
\end{equation}
which is trivially Hermitian and positive definite provided $v^{(b)}<0$. In this case we say that the irreducible representation $b$ is an
...
irreducible representations and thus works just as well for either.
...
only act on the explicit momentum-dependence in $S$, and neglecting the field dependence in the notation
