...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
The frequencies $\omega_n$ are called Matsubara frequencies and are defined by $\omega_n = (2n+1)\pi/\beta$ with $n\in\mathbb{Z}$ for fermions. For bosons we use in stead the notation
...
need to evaluate infinite  sums of Matsubara frequencies of the form $\sum_nh(i\omega_n)$, where $\omega_n$ is a Matsubara frequency, 
...
integration. We consider the complex contour integral along a path running counter-clockwise around the complex plane infinitesimally close to
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
	\def\cross[#1,#2,#3]{% Defines a diagonal cross with three arguments: x-pos, y-pos and size.
	  \draw [blue] (#1-#3/2,#2-#3/2) -- (#1+#3/2,#2+#3/2);
	  \draw [blue] (#1+#3/2,#2-#3/2) -- (#1-#3/2,#2+#3/2);
	}
    ...
    \draw [blue] (\x-0.1,-0.05) -- (\x, 0.05) (\x, 0.05) -- (\x+0.1,-0.05);
    % End branch cut
...
\end{tikzpicture}
\caption{Integration contour for the Matsubara sum $\sum_nh(i\omega_n)$. The contour is marked by a solid line and is imagined to continue
to $\pm i\infty$.
...
\begin{equation}
    ...
    n(z) = (e^{\beta z} - \zeta)^{-1},\quad z\in\mathbb{C},
\end{equation}
depending on whether the Matsubara
frequency in the sum is of fermionic ($\zeta = +1$) or bosonic ($\zeta = -1$) nature.

\begin{equation}
    \label{eq:Field:Mats:formula}
    \sum_{n\in\mathbb{Z}}\frac{1}{(i\omega_n - x)(i\omega_n - y)} = \frac{\beta}{x-y}\bigg(\frac{1}{1+e^{\beta x}} - \frac{1}{1+e^{\beta y}}\bigg).
\end{equation}
