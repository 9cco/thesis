...
A simpler expansion can be found when the bosonic fields corresponds to order parameter in a system close to a phase transition. In this case
we can assume the fields $\eta_\alpha$ to be small in general such that $S$ can be expanded about $0$. If the system is fermionic, then
the basonic field integral of the order-parameters will have resulted from a Hubbard-Stratonovich transformation


In general, when the bosonic fields in $Z(l)$ represents some order parameter, then assuming we are close to the critical point
then the fields must necessarily be small. An expansion about the stationary phase approximation then amounts to an expansion in the
bosonic fields.
